...
const DEFAULT_TIMEOUT_GRACE_MS = 5 * 60 * 1000;
...
export async function acquireSessionWriteLock(params: {
  ...
  const timeoutMs = resolvePositiveMs(params.timeoutMs, 30_000, { allowInfinity: true });
  const staleMs = resolvePositiveMs(params.staleMs, DEFAULT_STALE_MS);
  ...
  while (Date.now() - startedAt < timeoutMs) {
    ...
    try {
      ...
      // If the lock is held by the current PID but not in HELD_LOCKS (e.g. process restart
      // or library re-init in same process), reclaim it.
      if (inspected.pid === process.pid && inspected.pidAlive) {
        await fs.rm(lockPath, { force: true });
        continue;
      }
